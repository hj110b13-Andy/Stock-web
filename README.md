...
- 搜尋 / 篩選頁與首頁排行榜的股票清單：台股改為動態抓取 TWSE 官方上市公司清單（含真實產業別），美股則是約 100 檔精選跨產業大型股（`src/lib/data/universe.ts`）。清單數量刻意限制在一定檔數之內（`MAX_TW_UNIVERSE`）而不是涵蓋全部上千檔上市股票——批次查報價/技術訊號時同時打太多支股票給 TWSE，容易被限流，反而拖慢或打壞整個網站（包括跟這份清單無關的個股頁查詢）。之後若要擴大涵蓋範圍，建議搭配共用快取（見下方）與更保守的分批節流一起調整。
