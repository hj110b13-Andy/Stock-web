...
- **首頁**：AI 每日市場快報、我的關注清單、大盤指數概況、台股與美股焦點排行。
- **個股頁** `/stock/[symbol]`：即時（或近即時）報價、K 線圖 + 成交量（1個月/3個月/6個月/1年）、客觀技術訊號標籤（爆量、創新高/新低、均線、連漲跌天數）、關注清單星號。
- **搜尋 / 篩選** `/search`：台股、美股分開顯示兩個獨立區塊，各自可依產業、漲跌幅篩選與排序。
- **每日焦點榜單** `/highlights`：台股＋美股合併排名的漲幅榜／跌幅榜／成交量榜。
- **我的關注**：用瀏覽器 localStorage 儲存自選股清單，不需登入，首頁與個股頁可加入/移除。
- **AI 問答**：右下角浮動聊天視窗，可針對目前瀏覽的個股或任何代碼提問，回答會同時參考個股資料與台股＋美股大盤概況。

> 網站定位是「幫助使用者快速看懂資訊」，刻意不提供「建議買進/賣出」等操作建議——公開網站對外提供具體投資建議在台灣屬於受監管的證券投資顧問業務，未取得執照對外提供有法律風險。技術訊號、AI 快報皆為客觀數據描述，並非投資建議。
