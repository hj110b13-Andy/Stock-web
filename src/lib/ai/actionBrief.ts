  ...
  return cached(
    ...
    async () => {
      ...
      const system = [
        ...
        "籌碼面的詞彙實測特別容易漏解釋：『三大法人』第一次出現時一定要附帶解釋『（外資、投信、自營商這些大戶）』，『外資』第一次出現要附帶解釋『（外國機構投資人）』，『投信』要附帶『（國內基金公司）』，『融資』要附帶『（跟券商借錢買股票）』——這條規則優先於字數上限，就算為了塞這句解釋超過字數上限也要保留，同一篇裡第一次出現才需要附帶解釋，同一個詞後面重複出現不用每次都解釋一遍。",
        "如果提到『0軸』：注意原始資料裡的技術訊號常常已經自帶括號說明（例如『MACD黃金交叉（0軸上方，訊號較明確）』），不要把你自己要加的『0軸是判斷多空力道強弱的分界線』這句解釋硬塞進同一組括號或同一個子句裡跟原本的說明擠在一起，這樣容易寫出語句斷裂、讀不通的句子（實測出現過『0軸上方，判斷多空力道強隨著分界線』這種破碎文字）。正確做法是兩者分開：先照抄原始資料裡『0軸上方/下方，訊號較明確/力道較弱』這段，然後另外用一個完整的句子或子句補充『0軸是判斷多空力道強弱的分界線』，例如『MACD出現黃金交叉（0軸上方，訊號較明確；0軸是判斷多空力道強弱的分界線）』或拆成兩句話講，不要合併成一個文法破碎的插入語。",
