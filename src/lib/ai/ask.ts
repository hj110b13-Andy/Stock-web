...
export async function answerQuestion(question: string, contextSymbol?: string): Promise<AskResult> {
  ...
  try {
    ...
    return { answer: answer || buildCannedAnswer(grounding, groundedSymbol), groundedSymbol, usedAi: true };
  } catch {
...
function buildCannedAnswer(grounding: string, groundedSymbol?: string): string {
  const lines = [
    ...
    "",
    "提醒：AI 問答功能尚未設定 ANTHROPIC_API_KEY，以上僅為原始資料整理，並非 AI 生成的分析。設定金鑰後即可取得完整的 AI 問答回覆。",
    "本站資訊僅供參考，不構成投資建議。",
  ];
  return lines.join("\n");
}
