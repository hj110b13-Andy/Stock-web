...
- 網站已改為密碼保護的私人工具（見上方工作日誌），AI 問答會直接給推薦——**這個決定綁在密碼
  保護持續有效這個前提上**，見「重要慣例與限制」章節。
- 密碼保護上線後陸續回報並修好的問題：「密碼進不去」「AI 回覆有星號」「切換頁面很久」
  （`1c5c3d9`、`8bebfd9`），以及新增 5 項優化功能過程中自己測出來並修好的「到價提醒表單搞掛
  整個個股頁」無限渲染迴圈（`9264296`，React error #185，詳見下方工作日誌——**這個是本次對話
  自己新增功能時親手驗證抓到的真實 bug，不是使用者回報的**）。這些都是使用者當場回報或功能
  開發過程中逐一修正+正式站驗證，不是全新一輪全站巡檢，屬於 CLAUDE.md「後續只需複查已發現
  問題」精神的延伸。如果之後累積的零散小修正夠多，或又發現新的、不確定範圍的問題，記得評估
  要不要重新跑一次完整的規則二/三。
- 這次對話稍早已完整跑過一輪規則二/三：規則二自測修好 2 個 bug（`e95e963`），Opus 規則三第一輪
  又抓到 9 個資料正確性問題，全部自己修好（`9758888`），Opus 規則三第二輪複查全部確認修好。
- **上一輪 Opus 複查提出的「收盤後 TW 報價會不會又退回 0.00%」疑慮，已經驗證排除**：本次對話
  在台股收盤後（`已收盤`狀態）實際 curl `/api/search?market=TW`，卡在 0.00% 的數量是 14/199 檔
  （合理值，多數是真的沒成交的冷門股），跟盤中的比例相近，沒有復發成收盤前的 92/100 那種規模。
  這個殘留提醒可以視為已解決，不用再特別留意。
