...
**⚠️ 需要使用者動手（我這邊沒有權限代勞）：Gemini API 金鑰需要重新產生。** 見下方工作日誌
「Opus 規則三驗證挖出 2 個真實問題」那則——這組金鑰已經在正式站真實外洩過一次（呼叫失敗時
的錯誤訊息把金鑰顯示在聊天視窗裡，程式碼漏洞已修好，但已經外洩的這組金鑰本身沒辦法用程式
補救）。請到 [Google AI Studio](https://aistudio.google.com/apikey) 重新產生一組新的
`GEMINI_API_KEY`，並更新 Vercel 專案設定裡的環境變數。**在使用者確認做完這件事之前，接手
的裝置看到這裡都要提醒一次，不要假設已經處理過。**

**現況：籌碼面/重大訊息/基本面/新聞功能，以及 Opus 規則三挖出的金鑰外洩、股/張換算錯誤
兩個問題，都已經自己驗證過、也已經派第二輪 Opus 複查確認修好（100% 通過），流程走完，
可以回報「更新完成」。** 除了上面標記的「使用者需要重新產生 Gemini 金鑰」這件事還沒做之外，
沒有已知未修的 bug；這次對話（2026-09-11，很長一次）做完的其他事情都已經逐項驗證過，細節
見下方工作日誌，以下只列**還沒做、但已經評估過、之後有需要再處理**的項目：
