...
- 密碼保護上線後回報的「密碼進不去」「AI 回覆有星號」「切換頁面很久」都已修好並驗證
  （`1c5c3d9`、`8bebfd9`，見上方工作日誌）。**這三個問題是接連著上一輪對話修的，還沒有再走一輪
  完整的規則二/三地毯式檢查**——都是使用者當場回報、範圍明確的問題，逐一修正+正式站驗證，
  不是全新的一輪全站巡檢，屬於前面 CLAUDE.md 允許的「後續只需複查已發現問題」精神的延伸。
  如果之後累積的零散小修正夠多，或又發現新的、不確定範圍的問題，記得評估要不要重新跑一次
  完整的規則二/三。
